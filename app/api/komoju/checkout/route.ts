...
const PLANS: Record<string, { amount: number; currency: string; name: string }> = {
  standard: { amount: 2980, currency: "JPY", name: "スタンダードプラン（50件/月）" },
  business: { amount: 4980, currency: "JPY", name: "ビジネスプラン（無制限）" },
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const origin = req.headers.get("origin") || "https://google-review-ai.vercel.app";
